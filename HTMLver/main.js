...
//console.log(search);
    newarray = specs.filter(function (val) {
        console.log(val);
        if (val.code.includes(search) || val.name.includes(search) || val.condition.includes(search) || val.duration.includes(search)) {
            const newobj = { code : val.code, name : val.name, duration : val.duration, condition : val.condition };
            return newobj;
        }
    })
    showable(newarray);
